...
Показания счетчиков и данные о присутствии людей сохраняются в БД. С помощью дашборда в Grafana можно анализировать их изменения в реальном времени по периодам. Критические показатели вынесены на верхние панели, все остальное — в интерактивных графиках.

<img src="readme_images/app_ui/grafana_0.png">
Веб-интерфейс на Angular позволяет просматривать снимки со счетчиков, которые вызвали подозрения у системы. Это происходит, если:

- низкая уверенность модели в распознанной цифре;
- распознанное значение уменьшилось, хотя счетчик работает только на увеличение;
- зафиксирован резкий скачок показаний.

При выборе записи из истории пользователь может увидеть, как модель интерпретирует изображение, и вручную указать правильное значение. После этого исправленные снимки сохраняются в отдельную папку и в дальнейшем используются для дообучения модели.

<img src="readme_images/app_ui/manual_recognize.png">
...
Система построена по **микросервисной архитектуре** и состоит из двух независимых сервисов, общающихся через **Redis**:
...
│                                                                 │
│  ┌─────────────────────┐    Redis    ┌─────────────────────┐   │
│  │   Person Detector   │◄───────────►│  CNN Recognizer     │   │
│  │   (YOLOv8 + OpenCV) │             │   (PyTorch + CNN)   │   │
│  └─────────────────────┘             └─────────────────────┘   │
│           │                                    │                │
│           ▼                                    ▼                │
│  ┌─────────────────────┐    PostgreSQL ┌─────────────────────┐   │
...
1. **Скорость** — обмен данными между сервисами происходит за миллисекунды
2. **TTL (Time-To-Live)** — автоматическое удаление устаревших ключей (например, cooldown на 10 минут)
3. **Простота** — не нужно создавать сложные запросы к БД для межсервисного общения
4. **Атомарные операции** — безопасная запись/чтение из разных сервисов
...
### Базы данных
- **PostgreSQL** — основное хранилище (показания, логи)
- **Redis** — кэш и межсервисное взаимодействие
...
1. **Разработаны два независимых микросервиса:**
   - CNN Recognizer — распознавание показаний газового счетчика
   - Person Detector — трекинг присутствия человека

2. **Настроено взаимодействие между сервисами через Redis:**
   - Обмен ключами: gas_flow, last_seen, alert_triggered
   - Автоматическое управление TTL

3. **Создана полная Docker-инфраструктура:**
   - Общий базовый образ для ускорения сборки
   - Docker Compose для оркестрации 7+ контейнеров
   - Оптимизация размера образов

4. **Реализован веб-интерфейс на Angular:**
   - Просмотр подозрительных распознаваний
   - Ручная коррекция показаний

5. **Настроен мониторинг:**
