Вот улучшенная версия вашего кода с **распознаванием лиц** с помощью библиотеки `face_recognition`. Теперь система будет не просто отслеживать людей по ID, но и **идентифицировать их по лицам**!

## 🔍 Полный код с Face Recognition
            ...
            if results[0].boxes.id is not None:
                ...
                for i, track_id in enumerate(track_ids):
                    ...
                    if track_id not in self.entry_times:
                        ...
                        self.person_names[track_id] = name
                        last_recognition_time[track_id] = time.time()
                        
                        if name != "Unknown":
                            print(f"   ✅ Распознан как: {name} (уверенность: {face_conf:.2f})")
                        else:
                            print(f"   ❌ Лицо не распознано")
                    
                    # Периодическое распознавание (каждые 30 кадров)
                    elif frame_count % 30 == 0 and track_id not in last_recognition_time:
                        name, face_conf = self.identify_person(frame, boxes[i], track_id)
                        if name != "Unknown":
                            self.person_names[track_id] = name
...
project/
├── main.py
├── known_faces/           # Папка с фотографиями известных людей
│   ├── alice.jpg
│   ├── bob.jpg
│   └── charlie.jpg
├── face_encodings.pkl     # Сохраненные эмбеддинги лиц
├── tracking_log.csv       # Лог с именами
...
# или если проблемы с dlib:
pip install face-recognition
```

### 2. Добавьте фотографии известных людей:
```bash
# Создайте папку и добавьте фото
mkdir known_faces
# Добавьте фото с именем: alice.jpg, bob.jpg, charlie.jpg
```
...
| Функция | Описание |
|---------|----------|
| **Распознавание лиц** | Идентифицирует людей по лицу при появлении |
| **База знаний** | Сохраняет эмбеддинги лиц в `face_encodings.pkl` |
| **Цветовая маркировка** | Зеленый - известный человек, Красный - неизвестный |
| **Лог с именами** | CSV файл содержит имена и уверенность |
| **Автообучение** | Автоматически загружает или создает базу лиц |
| **Периодическое распознавание** | Перепроверяет лица для повышения точности |
...
```csv
Entry Time, Exit Time, Duration (s), Track ID, Person Name, Confidence
2023-12-15 14:30:22, 2023-12-15 14:30:45, 23.45s, ID:1, Alice, 0.92
2023-12-15 14:30:25, 2023-12-15 14:30:40, 15.23s, ID:2, Unknown, 0.00
2023-12-15 14:30:30, 2023-12-15 14:30:50, 20.12s, ID:3, Bob, 0.87
```
...
1. Добавьте **несколько фото** одного человека для лучшей точности
2. Используйте фотографии **хорошего качества** с четким лицом
3. Увеличьте порог уверенности в коде (строка 55-56)

**Оптимизация производительности:**
```python
# Распознавание реже (каждые 60 кадров)
elif frame_count % 60 == 0:
    # ...
```

**Добавление новых лиц во время работы:**
```python
# Вы можете добавить функцию для динамического добавления лиц:
